...
void main(void) {
  ...
  // Brand brown/gold nebula background
  vec3 col=vec3(0);
  float bg=clouds(vec2(st.x+T*.3,-st.y));
  vec3 bgCol=vec3(bg*.18, bg*.10, bg*.04);
  col=bgCol;
  ...
  for (float i=1.; i<12.; i++) {
    ...
    // Gold/amber swirls
    vec3 c1=vec3(.83, .66, .33); // #D4A853 gold
    vec3 c2=vec3(.96, .90, .83); // #F5E6D3 cream
    vec3 c3=vec3(.77, .60, .42); // #C49A6C brand-400

    col+=.0015/d*(cos(sin(i)*mix(c1,c2,sin(i)*.5+.5))+1.);
    float b=noise(i+p+bg*1.731);
    col+=.003*b/length(max(p,vec2(b*p.x*.02,p.y)));
    col=mix(col, vec3(bg*.22, bg*.12, bg*.04), d);
  }

  // Subtle golden glow at center
  float glow=exp(-length(uv)*1.5);
  col+=vec3(.83,.66,.33)*glow*.15;
